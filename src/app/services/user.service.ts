...
export class UserService {
  ...
  private userSignal = signal<User[]>(this.Users); // siganl for usersList

  // pagination signals

  private pageSizeSignal = signal<number>(10);
  private currentPageSizeSignal = signal<number>(1);

  //find tottal pages for implementing pagination

  public totalPages = computed(() => {
    return Math.ceil(this.userSignal().length / this.pageSizeSignal());
  });
  ...
  public paginatedUsers = computed(() => {
    const startIndex =
      (this.currentPageSizeSignal() - 1) * this.pageSizeSignal();
  ...
  setPageSize(size: number): void {
    ...
    this.currentPageSizeSignal.set(1);
  }

  setCurrentPages(page: number): void {
    if (page >= 1 && page <= this.totalPages()) {
      this.currentPageSizeSignal.set(page);
    }
  }

  nextPage(): void {
    if (this.currentPageSizeSignal() < this.totalPages()) {
      this.currentPageSizeSignal.update((page) => page + 1);
    }
  }

  previousPage(): void {
    if (this.currentPageSizeSignal() > 1) {
      this.currentPageSizeSignal.update((page) => page - 1);
    }
  }
  ...
  get Currentpage() {
    return this.currentPageSizeSignal();
  }
